          ...
          <div className="grid md:grid-cols-3 gap-12">
            {instructors.map((instructor, index) => (
              <motion.div key={index} className="group" initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: index * 0.1 }}>
                <div className={`relative overflow-hidden mb-6 aspect-[3/4] ${index === 0 ? 'bg-white' : ''}`}>
                  <img src={instructor.image} alt={instructor.name} className={`w-full h-full transition-all duration-500 ${index === 0 ? 'object-contain p-6' : 'object-cover'}`} />
                </div>
